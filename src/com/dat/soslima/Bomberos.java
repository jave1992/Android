...
public class Bomberos extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.bomberos_layout);
		
		ActionBar actionBar=getActionBar();
		actionBar.setDisplayHomeAsUpEnabled(true);
		

		lv=(ListView)findViewById(R.id.listViewBomberos);
		editText=(EditText)findViewById(R.id.edBombero);
		
		UtilDB db = UtilDB.GetUtilDb(this);
		db.openDataBase();
		Cursor c=db.getDataBase().rawQuery("SELECT * FROM bombero",null);
		...
		if(c.moveToFirst()){
		    ...
		    lv.setOnItemClickListener(new OnItemClickListener(){
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					try{
						...
						if(estado){
							if(lista2.get(position).getSubtitulo().length()==3){
								intent.setData(Uri.parse("tel:"+lista2.get(position).getSubtitulo()));
							}else{
								intent.setData(Uri.parse("tel:01"+lista2.get(position).getSubtitulo()));
							}
						}else{
							if(lista.get(position).getSubtitulo().length()==3){
								intent.setData(Uri.parse("tel:"+lista.get(position).getSubtitulo()));
							}else{
		    ...
		    editText.addTextChangedListener(new TextWatcher(){
				...
				public void onTextChanged(CharSequence s, int start,
					...
					lv.setAdapter(new AdapterTitulo(Bomberos.this,lista2));
				}
		    });
		}
	}
