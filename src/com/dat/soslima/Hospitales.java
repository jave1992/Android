...
public class Hospitales extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.hospitales_layout);
		
		ActionBar actionBar=getActionBar();
		actionBar.setDisplayHomeAsUpEnabled(true);
		
		lv=(ListView)findViewById(R.id.listViewHospital);
		editText=(EditText)findViewById(R.id.edHospital);
		
		UtilDB db = UtilDB.GetUtilDb(this);
		db.openDataBase();
		Cursor c=db.getDataBase().rawQuery("select * from hospital",null);
		...
		if(c.moveToFirst()){
		    ...
		    editText.addTextChangedListener(new TextWatcher(){
				...
				public void onTextChanged(CharSequence s, int start,
					...
					lv.setAdapter(new AdapterTitulo(Hospitales.this,lista2));
				}
		    });
		}	
	}
